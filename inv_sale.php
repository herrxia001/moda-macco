		...
		<div class="row">
			...
			<table id="table" class="table-sm" data-toggle="table" data-single-select="true" data-click-to-select="true">
				<thead class="thead-light">
					...
					<th class="p-1" data-field="idx_data" data-width="40" data-width-unit="%"><?php echo $thisResource->comProduct ?></th>
					<th class="p-1" data-field="idx_price" data-width="10" data-width-unit="%">售价</th>
					<th class="p-1" data-field="idx_discount" data-width="15" data-width-unit="%">促销</th>
...
<!-- Modal: modalSale -->
<div class="modal fade" id="modalSale" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-body">
				<!-- top menu -->		
				<div class="row">
					<div class="p-1 col-6">
						<b>促销:</b> <label id="m_i_code" class="ml-2 mt-2" style="font-size: 20px; font-weight: bold"></label>
					</div>
					<div class="p-1 col-6" align="right">
						<button type="button" class="btn btn-secondary" onclick="cancelSale()"><span class="fa fa-times"></span></button>
						<button type="button" class="mr-1 btn btn-primary" style="width:60px" onclick="doneSale()"><span class="fa fa-check"></span></button>
					</div>
				</div>
				<div class="row">
				...
				<div class="row">
					<div class="col-8 p-1">
						<div class="p-1 input-group">
							...
							<input type="text" class="form-control" name="m_i_name" id="m_i_name" readonly="">
						</div>
						<div class="p-1 input-group">
							<div class="input-group-prepend"><span class="input-group-text">库存</span></div>
							<input type="text" class="form-control" name="m_old_count" id="m_old_count" readonly="">
						</div>
						<div class="p-1 input-group">
							<div class="input-group-prepend"><span class="input-group-text">售价</span></div>
							<input type="number" min="0" step="0.01" class="form-control" name="m_price" id="m_price" readonly="">
						</div>
						<div class="p-1 input-group">
					...
					<div class="col-4 p-1 align-self-center" align="center">
						<img id="img" src= "blank.jpg" style="max-width: 100%;" />
					</div>
				</div>
			</div>
		</div>
...
$(document).ready(function(){
	// display Title
	document.getElementById("myTitle").innerHTML = '促销';
...
function searchAll() {
	...
	setTimeout(
    function() {
		getRequest("api/getInvsDiscount.php", searchYes, searchNo);
    }, 500);
}
function displaySum(){
...
function loadTable(){
	...
	for(var i=0; i<invs.length; i++){
		...
		rows.push({
			i_code: invs[i]['i_code'],
			idx_image: imgStr,
			idx_data: dataStr,
			idx_price: invs[i]['price'],
			idx_discount: parseInt(invs[i]['discount']) + "% ("+((invs[i]['price'] * (100 - invs[i]['discount']))/100).toFixed(2)+")",
			idx_count: countStr,
...
/************************************************************************************
	SEARCH
************************************************************************************/
function searchInv(code) {
	var link = "api/getInvByicode.php?i_code="+code;
	getRequest(link, afterSearchInvYes, afterSearchInvNo);
}
function afterSearchInvYes(result){
	var res = result[0];
	console.log(res);
	$("#m_i_code").text(res["i_code"]);
	$("#m_i_name").val(res["i_name"]);
	$("#m_old_count").val(res["count"]);
	$("#m_price").val(res["price"]);
	$("#m_discount").val("");
	if(res["discount"] != "" && res["discount"] != "0.00")
		$("#m_discount").val(parseInt(res["discount"]));
	var imgSrc = "blank.jpg";
	if(res['m_no'] != null)
        imgSrc = res['path']+"/"+res['i_id']+"_"+res['m_no']+"_s.jpg";
	$("#img").attr("src",imgSrc);
	refreshSale();
	$modalSale.modal();
}
...
function cancelSale(){
	$modalSale.modal("toggle");
}
function doneSale(){
	var form = new FormData();
	form.append('i_code', $("#m_i_code").text());
	form.append('discount', $("#m_discount").val());
	$modalSale.modal("toggle");
	postRequest('api/postInvSale.php', form, submitok, null);
}
function submitok(result){
	searchAll();
}
function afterSearchInvNo(){
	alert("没有找到");
}
// This is a callback for autocomplete
function doneAutocomp() {
	searchInv(document.getElementById("i_code").value);
}
/************************************************************************************
	FUNCTIONS
************************************************************************************/
